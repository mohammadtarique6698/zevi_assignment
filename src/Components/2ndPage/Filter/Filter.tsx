      ...
      .filter((item) => {
        if (brandFilters.length === 0) {
          ...
          return (
            ...
            (brandFilters.includes("Others") &&
              !["H&M", "Mango"].includes(item.company_Name))
          );
        }
      })
      .filter(
        (item) =>
          ...
          priceFilters.includes(getPriceRange(item.updated_price))
      )
      .filter(
        (item) =>
          ratingFilters.length === 0 || ratingFilters.includes(item.rating)
      );
